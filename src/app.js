...
function chatPanel(){const c=currentConversation();return `<section class="chat-panel"><div class="mentor"><img src="/assets/mentorAvatar.webp" alt="달빛 도령"><div><h2>달빛 도령</h2><span>${aiCoach?'달빛 사주 AI 상담':'문맥을 이어가는 체험 코칭'}</span></div><button class="plain" data-action="chat-history">이전 상담</button></div><p class="chat-note">${aiCoach?'사주와 대화 내용을 함께 읽고 답합니다. 사람 상담사가 아니며, 건강·투자·법률 판단은 해당 전문가에게 확인해 주세요.':'AI·전문 상담사와의 실시간 대화가 아닙니다. 사주와 입력한 상황을 활용한 규칙 기반 코칭이며, 건강·투자·법률 판단은 해당 전문가에게 확인해 주세요.'}</p><div class="messages" role="log" aria-live="polite">${(c?.messages||[]).map(m=>capturing?`<article class="message ${m.role} pickable${capturing.has(m.id)?' picked':''}"><button class="pick" data-pick="${m.id}" aria-pressed="${capturing.has(m.id)}"><span class="tick" aria-hidden="true"></span><span class="bubble">${esc(m.text)}</span></button></article>`:`<article class="message ${m.role}"><div class="bubble">${esc(m.text)}</div>${m.role==='assistant'?`<div class="answer-tools"><time>${DateTime.fromISO(m.at).toFormat('HH:mm')}</time><button data-copy="${m.id}">복사</button><button data-save-answer="${m.id}">${data.savedAnswers.some(a=>a.messageId===m.id)?'저장됨':'저장'}</button><button data-share="${m.id}">공유</button></div>`:`<time>${DateTime.fromISO(m.at).toFormat('HH:mm')}</time>`}</article>`).join('')}${(c?.pending?'<article class="message assistant pending"><div class="bubble"><i></i><i></i><i></i><span class="sr-only">답변을 작성하고 있어요</span></div></article>':'')}</div>${capturing?`<div class="capture-bar"><span>${capturing.size}개 선택됨</span><button class="plain" data-capture="cancel">취소</button><button class="primary" data-capture="save" ${capturing.size?'':'disabled'}>이미지로 저장</button></div>`:''}<div class="quick-topics"${capturing||(c?.messages||[]).length>1?' hidden':''}>${p().topics.map(t=>`<button data-quick="${t} 고민이 있어요">${t} 이야기</button>`).join('')}</div><form id="chat-form"${capturing?' hidden':''}><label class="sr-only" for="chat-input">상담 메시지</label><textarea id="chat-input" name="message" rows="1" maxlength="1500" placeholder="무엇이 고민이신가요?"></textarea><button class="send" aria-label="메시지 보내기">보내기</button></form></section>`;}
...
// 고른 말풍선을 대화 순서대로 모아 이미지로 만듭니다. 공유 시트를 쓸 수
// 있으면 파일로 넘기고, 아니면 내려받습니다.
async function saveCapture(){
 const c=currentConversation(),picked=(c?.messages||[]).filter(m=>capturing.has(m.id));
 ...
 try{
  ...
  const file=new File([blob],'달빛사주-상담.png',{type:'image/png'});
  if(navigator.canShare?.({files:[file]})){await navigator.share({files:[file],title:'달빛 사주 상담'});}
  else{const u=URL.createObjectURL(blob);const a=document.createElement('a');a.href=u;a.download='달빛사주-상담.png';a.click();
   setTimeout(()=>URL.revokeObjectURL(u),1000);notice('이미지로 저장했어요.');}
  capturing=null;render();
 }catch(e){if(e.name!=='AbortError')notice('이미지를 만들지 못했어요. 다시 시도해 주세요.');}
}
