...
function welcome(){return `<section class="welcome welcome-with-auth"><img class="landscape" src="/assets/splash.webp" fetchpriority="high" decoding="async" alt="보름달 아래 산과 호수"><span class="vertical">언제나, 당신의 곁에</span><div class="welcome-copy"><img class="mark" src="/assets/mark.webp" alt="달빛 사주"><h1>나를 읽고,<br>내일을 묻다</h1><p>천년의 지혜가 오늘의 당신과 만납니다.</p><footer class="welcome-auth" aria-label="로그인 및 이용 안내">${authUser?`<button class="primary gold" data-nav="${p()?'result':'birth'}">${p()?'나의 사주 이어보기':'사주 시작하기'}</button><button class="welcome-link" data-nav="settings">내 보관함 · 로그아웃</button><small>${esc(authUser.name||'회원')}님 · 로그인되어 있어요</small>`:`${authStatus==='loading'?'<small role="status">로그인 연결을 확인하고 있어요.</small>':authStatus==='error'?'<small role="alert">로그인 연결을 확인하지 못했어요.</small><button class="welcome-link" data-auth-refresh>연결 다시 확인</button>':''}<div class="auth-buttons">${['google','apple'].map(provider=>`<button class="auth ${provider}" ${authOptions[provider]?'data-login="'+provider+'"':'disabled'}>${provider==='google'?'Google':'Apple'}로 계속하기${!authOptions[provider]&&authStatus==='ready'?' · 준비 중':''}</button>`).join('')}</div><button class="welcome-link" data-guest-start>로그인 없이 이용하기</button>`}<div class="welcome-legal"><a href="/terms/">이용약관</a><span>·</span><a href="/privacy/">개인정보처리방침</a></div></footer></div></section>`;}
