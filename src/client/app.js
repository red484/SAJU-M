...
let authUser=null,authOptions={google:false,apple:false},authStatus='loading';
...
function welcome(){return `<section class="welcome welcome-with-auth"><img class="landscape" src="/assets/splash.webp" fetchpriority="high" decoding="async" alt="보름달 아래 산과 호수"><span class="vertical">언제나, 당신의 곁에</span><div class="welcome-copy"><img class="mark" src="/assets/mark.webp" alt="달빛 사주"><h1>나를 읽고,<br>내일을 묻다</h1><p>천년의 지혜가 오늘의 당신과 만납니다.</p><footer class="welcome-auth" aria-label="로그인 및 이용 안내">${authUser?`<button class="primary gold" data-nav="${p()?'result':'birth'}">${p()?'나의 사주 이어보기':'사주 시작하기'}</button><button class="welcome-link" data-action="logout">로그아웃</button><button class="welcome-link" data-nav="settings">내 보관함 보기</button><small>${esc(authUser.name||'회원')}님 · 로그인되어 있어요</small>`:`${authStatus==='loading'?'<small role="status">로그인 연결을 확인하고 있어요.</small>':authStatus==='error'?'<small role="alert">로그인 연결을 확인하지 못했어요.</small><button class="welcome-link" data-auth-refresh>연결 다시 확인</button>':''}${authFailure?'<p class="auth-failure" role="alert">로그인을 완료하지 못했어요. 잠시 후 다시 시도하거나 Google로 로그인해 주세요.</p>':''}<div class="auth-buttons">${['google','apple'].map(provider=>`<button class="auth ${provider}" ${authOptions[provider]?'data-login="'+provider+'"':'disabled'}>${provider==='google'?'Google':'Apple'}로 계속하기${!authOptions[provider]&&authStatus==='ready'?' · 준비 중':''}</button>`).join('')}</div><button class="welcome-link" data-guest-start>로그인 없이 이용하기</button>`}<div class="welcome-legal"><a href="/terms/">이용약관</a><span>·</span><a href="/privacy/">개인정보처리방침</a></div></footer></div></section>`;}
...
function settingsPage(){
 const account=authUser
  ...
  :`<p>지금 로그인하면 이 브라우저의 기존 기록이 계정에 연결됩니다. 다른 기기에서도 같은 계정으로 불러올 수 있어요.</p><div class="auth-buttons"><button class="auth apple" ${authOptions.apple?'data-login="apple"':'disabled'}>${authOptions.apple?'Apple로 계속하기':'Apple 로그인 준비 중'}</button><button class="auth google" ${authOptions.google?'data-login="google"':'disabled'}>${authOptions.google?'Google로 계속하기':'Google 로그인 준비 중'}</button></div>${!authOptions.apple||!authOptions.google?'<p class="hint">버튼을 활성화하려면 서버에 각 제공자의 OAuth 환경변수를 설정해야 합니다.</p>':''}`;
...
async function action(a){if(a==='epic'){if(await engineReady())await loadEpic();return;}if(a==='retry'){if(!loaded)await load();else await persist();return;}if(a==='logout'){try{const {response}=await logout();if(!response.ok)throw new Error();authUser=null;page='welcome';await load();notice('로그아웃했어요.');}catch{notice('로그아웃하지 못했어요. 다시 시도해 주세요.');}return;}if(a==='export-json'){download('달빛-보관함.json',JSON.stringify(data,null,2),'application/json');return;}if(a==='export-text'){download('달빛-보관함.txt',exportText());return;}if(a==='chat-history'){chatHistoryModal();return;}
