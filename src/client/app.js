...
function settingsPage(){
 const account=authUser
  ...
  :`<p>지금 로그인하면 이 브라우저의 기존 기록이 계정에 연결됩니다. 다른 기기에서도 같은 계정으로 불러올 수 있어요.</p><div class="auth-buttons">${authOptions.apple?'<button class="auth apple" data-login="apple">Apple로 계속하기</button>':''}${authOptions.google?'<button class="auth google" data-login="google">Google로 계속하기</button>':''}${!authOptions.apple&&!authOptions.google?'<p class="hint">운영 서버에 Apple·Google 로그인 설정이 아직 연결되지 않았습니다.</p>':''}</div>`;
...
async function requestAi(c, messages){try{const {response:r,body:j}=await requestCoach({chart:chartFor(c),messages});if(r.ok&&j.text)return {ok:true,text:j.text,source:j.source,offer:j.offer};return {ok:false,reason:r.status===429?'AI 사용량이 많아 기본 답변을 표시했습니다. 잠시 뒤 다시 시도할 수 있어요.':'AI가 20초 안에 답하지 못해 기본 답변을 유지했습니다. 다시 시도할 수 있어요.'};}catch(e){return {ok:false,reason:e.name==='AbortError'?'AI가 20초 안에 답하지 못해 기본 답변을 유지했습니다. 다시 시도할 수 있어요.':'연결 문제로 기본 답변을 표시했습니다. 다시 시도할 수 있어요.'};}}
