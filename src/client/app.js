...
let data={profile:null,records:[],conversations:[],savedAnswers:[]},revision=0,result=null,page=location.pathname==='/settings'?'settings':'welcome',loaded=false,saveState='loading',activeConversation=null,month=now().toFormat('yyyy-MM'),selectedDate=now().toISODate(),recordFilter='all',pendingProfile=null,saveQueue=Promise.resolve(),generation=0;
let savedBaseline=null,saveError='';
let authUser=null,authOptions={google:false,apple:false};
...
function welcome(){return `<section class="welcome welcome-entry"><img class="landscape" src="/assets/splash.webp" fetchpriority="high" decoding="async" alt="보름달 아래 산과 호수"><div class="entry-brand"><img class="mark" src="/assets/mark.webp" alt=""><p class="entry-name">달빛 사주</p><h1>당신의 이야기에<br>달빛을 비춰드릴게요.</h1><p>${authUser?'다시 만나 반가워요. 나의 이야기를 이어가세요.':'로그인하고 나의 사주와 상담 기록을 이어가세요.'}</p></div><div class="entry-actions">${authUser?`<p class="entry-account">${esc(authUser.name||authUser.email||'회원')}님 · 로그인되어 있어요</p><button class="primary gold" data-nav="${p()?'result':'birth'}">${p()?'나의 사주 이어보기':'사주 시작하기'}</button><button class="welcome-link" data-nav="settings">내 보관함 · 로그아웃</button>`:`<div class="auth-buttons"><button class="auth google" ${authOptions.google?'data-login="google"':'disabled'}>Google로 계속하기${authOptions.google?'':' · 준비 중'}</button><button class="auth apple" ${authOptions.apple?'data-login="apple"':'disabled'}>Apple로 계속하기${authOptions.apple?'':' · 준비 중'}</button></div><button class="welcome-link" data-guest-start>로그인 없이 이용하기</button>`}<p class="entry-legal"><a href="/terms/">이용약관</a><span>·</span><a href="/privacy/">개인정보 처리방침</a></p></div></section>`;}
...
async function loadAuth(){try{const [providers,me]=await Promise.all([authProviders(),currentUser()]);if(providers.response.ok)authOptions=providers.body;if(me.response.ok)authUser=me.body.user||null;}catch{authUser=null;}if(loaded)render();}
render();
