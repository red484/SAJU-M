...
import {authProviders,currentUser,logout,deleteAccount,loginUrl,loginWithToss,clearTossLogin,isAppsInToss} from './api/auth.js';
...
let data={profile:null,records:[],conversations:[],savedAnswers:[]},revision=0,result=null,page=new URLSearchParams(location.search).get('onboarding')==='1'?'birth':location.pathname==='/settings'?'settings':'welcome',loaded=false,saveState='loading',activeConversation=null,month=now().toFormat('yyyy-MM'),selectedDate=now().toISODate(),recordFilter='all',pendingProfile=null,saveQueue=Promise.resolve(),generation=0;
...
const PLUSICO={
 ...
 capture:ico('<rect x="3.2" y="6.2" width="17.6" height="13.6" rx="2.4"/><circle cx="12" cy="13" r="3.4"/><path d="M8.6 6.2l1.3-2.1h4.2l1.3 2.1"/>'),
 history:ico('<circle cx="12" cy="12" r="8.4"/><path d="M12 7.3V12l3.1 2"/>'),
 fresh:ico('<path d="M4.2 19.8l1.1-4 9.4-9.4a2 2 0 012.8 0l.1.1a2 2 0 010 2.8l-9.4 9.4z"/><path d="M14 7.2l2.8 2.8"/>'),
 record:ico('<path d="M6.4 3.6h11.2v16.8l-5.6-3.3-5.6 3.3z"/>')};
...
function persist(){if(!loaded){notice('저장소 연결 후 다시 시도해 주세요.');return Promise.resolve(false);}const g=generation,snapshot=JSON.parse(JSON.stringify(data));saveState='saving';refreshSave();refreshSaveError();saveQueue=saveQueue.then(async()=>{if(g!==generation)return false;try{
  let saved=snapshot;
  let savedReply=await putJournal(snapshot,revision),res=savedReply.response;
  if(res.status===409){
    const latest=await getJournal();if(!latest.response.ok)throw new Error('최신 기록을 불러오지 못했습니다.');
    const remote=latest.body,merged=remote.revision===0&&!remote.data?snapshot:mergeJournal(savedBaseline,snapshot,remote.data);
    ...
    saved=merged;
    savedReply=await putJournal(merged,remote.revision);res=savedReply.response;
    if(res.ok){Object.assign(data,current);notice('다른 탭의 변경 내용과 합쳐 저장했습니다.');}
  }
  const reply=savedReply.body;if(!res.ok)throw new Error(reply.error);
...
function chatPanel(){const c=currentConversation();return `<section class="chat-panel"><div class="mentor"><img src="/assets/mentorAvatar.webp" alt="달빛 도령"><div><h2>달빛 도령</h2><span>${aiCoach?'달빛 사주 AI 상담':'문맥을 이어가는 체험 코칭'}</span></div><button class="plain" data-action="chat-history">이전 상담</button></div><p class="chat-note">${aiCoach?'사주와 대화 내용을 함께 읽고 답합니다. 사람 상담사가 아니며, 건강·투자·법률 판단은 해당 전문가에게 확인해 주세요.':'AI·전문 상담사와의 실시간 대화가 아닙니다. 사주와 입력한 상황을 활용한 규칙 기반 코칭이며, 건강·투자·법률 판단은 해당 전문가에게 확인해 주세요.'}</p><div class="messages" role="log" aria-live="polite">${(()=>{const picked=capturedIds();return (c?.messages||[]).map(m=>capturing?`<article class="message ${m.role} pickable${picked.includes(m.id)?' picked':''}"><button class="pick" data-pick="${m.id}" aria-pressed="${picked.includes(m.id)}"><span class="tick" aria-hidden="true"></span><span class="bubble">${esc(m.text)}</span></button></article>`:`<article class="message ${m.role}${m.streaming?' composing':''}"><div class="bubble">${esc(m.text)}</div>${m.role==='assistant'?`<div class="answer-tools"><time>${DateTime.fromISO(m.at).toFormat('HH:mm')}</time>${m.streaming?'<span class="ai-working" role="status"><span aria-hidden="true"></span>답변을 다듬고 있어요</span>':`<button data-copy="${m.id}">복사</button><button data-save-answer="${m.id}">${data.savedAnswers.some(a=>a.messageId===m.id)?'저장됨':'저장'}</button>`}</div>`:`<time>${DateTime.fromISO(m.at).toFormat('HH:mm')}</time>`}</article>`).join('');})()}</div>${capturing?(()=>{const n=capturedIds().length,off=n?'':' disabled';return `<div class="capture-bar"><div class="capture-guide"><span>${captureGuide()}</span><button class="plain" data-capture="reset"${off}>선택 해제</button></div><div class="capture-acts">${canCopyImage()?`<button data-capture="copy"${off}>복사</button>`:''}${canShareFiles()?`<button data-capture="share"${off}>공유</button>`:''}<button data-capture="save"${off}>저장</button><button class="plain" data-capture="cancel">취소</button></div></div>`;})():''}${(()=>{const cold=(c?.messages||[]).length<2;
 const chips=`<div class="quick-topics">${p().topics.map(t=>`<button data-quick="${t} 고민이 있어요">${t} 이야기</button>`).join('')}</div>`;
 const sheet=capturing||!plusOpen?'':`<div class="plus-sheet" id="plus-sheet"><div class="plus-acts">${[['capture','대화 캡처',PLUSICO.capture],['history','이전 상담',PLUSICO.history],['fresh','새 상담',PLUSICO.fresh],['record','선택 기록',PLUSICO.record]].map(([k,label,d])=>`<button data-plus-do="${k}"><span>${d}</span>${label}</button>`).join('')}</div>${cold?'':chips}</div>`;
 return sheet+(capturing||!cold?'':chips);})()}<form id="chat-form"${capturing?' hidden':''}><button type="button" class="plus${plusOpen?' on':''}" data-plus aria-expanded="${plusOpen}" aria-controls="plus-sheet" aria-label="상담 도구 열기">${PLUSICO.plus}</button><label class="sr-only" for="chat-input">상담 메시지</label><textarea id="chat-input" name="message" rows="1" maxlength="1500" placeholder="무엇이 고민이신가요?"></textarea><button class="send" aria-label="메시지 보내기">보내기</button></form></section>`;}
...
function settingsPage(){
 const account=authUser
  ?`<div class="account-state"><b>${esc(authUser.name||authUser.email||'로그인된 사용자')}</b><span>${esc(authUser.email||'계정에 연결됨')}</span></div><p>이 보관함은 계정에 연결되어 다른 기기에서도 다시 불러올 수 있습니다.</p><button class="secondary" data-action="logout">로그아웃</button>`
  :`<p>지금 로그인하면 이 브라우저의 기존 기록이 계정에 연결됩니다. 다른 기기에서도 같은 계정으로 불러올 수 있어요.</p><div class="auth-buttons">${authButtons()}</div>${isAppsInToss()?!authOptions.toss?'<p class="hint">Toss 로그인 연결을 준비하고 있어요.</p>':'':!authOptions.apple||!authOptions.google?'<p class="hint">버튼을 활성화하려면 서버에 각 제공자의 OAuth 환경변수를 설정해야 합니다.</p>':''}`;
 return `<div class="narrow"><p class="eyebrow">나의 이야기, 나의 권한</p><h1>개인 보관함</h1><section class="card account-card"><h2>계정과 동기화</h2>${account}</section><section class="card"><h2>어디에 저장되나요?</h2><p>프로필·상담·선택 기록·저장한 답변은 이 서비스의 서버 보관함에 저장됩니다. ${authUser?'로그인 계정과 연결되어 있습니다.':'로그인 전에는 이 브라우저의 보안 쿠키로 연결됩니다.'}</p><p>AI 상담을 이용하면 답변 생성에 필요한 명식과 계산된 연·월 흐름, 현재 대화와 관련된 과거 대화·선택 기록·온보딩 고민이 Cafe24 LLM Router로 전송될 수 있습니다. 운영 서버 관리자 접근을 막는 종단간 암호화 서비스는 아닙니다.</p><button class="secondary" data-action="retry" data-save-status>${storageLabel()}</button></section><section class="card"><h2>내 기록 가져가기</h2><p>프로필·상담·선택 기록을 JSON 또는 읽기 쉬운 문서로 내보냅니다. 파일에는 개인정보가 포함됩니다.</p><div class="button-row"><button class="secondary" data-action="export-json">JSON 내보내기</button><button class="secondary" data-action="export-text">문서 내보내기</button></div></section><section class="card"><h2>저장한 상담 답변</h2>${data.savedAnswers.length?data.savedAnswers.map(a=>`<details><summary>${esc(a.text.slice(0,45))}</summary><p class="preline">${esc(a.text)}</p><button class="plain" data-remove-answer="${a.id}">저장 해제</button></details>`).join(''):'<p class="muted">상담 답변 아래의 ‘저장’을 누르면 이곳에 모입니다.</p>'}</section><section class="card"><h2>연결 상태</h2><p>체험 코칭: 사용 가능<br>실제 AI 상담: ${aiCoach?'연결됨':'미연결'}<br>계정 동기화: ${authUser?'연결됨':'로그인 전'}<br>캘린더 회고 일정: 파일 가져오기 방식</p></section><section class="card legal-links"><h2>정책과 고객지원</h2><div class="legal-link-list"><a href="/privacy/">개인정보 처리방침</a><a href="/terms/">서비스 이용약관</a><a href="/support/">고객지원</a><a href="/account-deletion/">계정 및 데이터 삭제 안내</a></div></section><section class="card danger-zone"><h2>${authUser?'회원 탈퇴':'개인정보 전체 삭제'}</h2><p>${authUser?'계정과 서버에 저장된 모든 보관함 정보를 삭제합니다.':'현재 브라우저의 보관함 정보를 삭제합니다.'} 삭제 후 복구할 수 없습니다.</p><button class="danger" data-action="delete-all">${authUser?'계정과 모든 정보 삭제':'내 정보와 기록 모두 삭제'}</button></section></div>`;
}
...
function chatHistoryModal(){
 const conversations=data.conversations.slice().reverse();
 openModal(`<h2>이전 상담 이어가기</h2><button class="secondary" id="new-chat">새 상담 시작하기</button><div class="conversation-list">${conversations.length?conversations.map(c=>`<div class="conversation-row"><button class="record-row" data-conversation="${c.id}"><b>${esc(c.profileName)} · ${esc(c.topic||'상담')}</b><span>${dateLabel(c.created.slice(0,10))} · ${c.messages.length}개 메시지 ${c.profileKey!==profileKey()?'· 이전 사주 (읽기 전용)':''}</span></button><button class="conversation-delete" data-delete-conversation="${c.id}" aria-label="${esc(c.profileName)}의 ${esc(c.topic||'상담')} 상담 삭제">삭제</button></div>`).join(''):'<p class="muted">아직 저장된 상담이 없어요.</p>'}</div>`);
 $('#new-chat').onclick=()=>{activeConversation=null;$('#modal').close();render();persist();};
 document.querySelectorAll('[data-conversation]').forEach(b=>b.onclick=()=>{activeConversation=b.dataset.conversation;$('#modal').close();render();});
 document.querySelectorAll('[data-delete-conversation]').forEach(b=>b.onclick=()=>{
  const id=b.dataset.deleteConversation,c=data.conversations.find(v=>v.id===id);if(!c)return;
  openModal(`<h2>이 상담을 삭제할까요?</h2><p>${dateLabel(c.created.slice(0,10))}의 ${esc(c.topic||'상담')} 대화가 모두 삭제되며 복구할 수 없어요. 따로 저장한 상담 답변은 보관함에 남습니다.</p><button class="danger" id="confirm-delete-conversation">상담 삭제</button><button class="secondary" id="cancel-delete-conversation">취소</button>`);
  $('#cancel-delete-conversation').onclick=chatHistoryModal;
  $('#confirm-delete-conversation').onclick=async()=>{data.conversations=data.conversations.filter(v=>v.id!==id);if(activeConversation===id)activeConversation=null;await persist();$('#modal').close();render();notice('상담을 삭제했어요.');};
 });
}
...
document.addEventListener('click',async e=>{const button=e.target.closest('[data-login]');if(!button)return;const provider=button.dataset.login;if(provider!=='toss'){location.href=loginUrl(provider);return;}tossLoginBusy=true;render();try{const {body}=await loginWithToss();authUser=body.user||null;await load();page=p()?'today':'birth';notice('토스 계정으로 로그인했어요.');}catch(error){notice(error.message||'토스 로그인을 완료하지 못했어요.');}finally{tossLoginBusy=false;render();}});
document.addEventListener('click',e=>{
 if(e.target.closest('[data-auth-refresh]')){authStatus='loading';render();loadAuth();}
});
document.addEventListener('click',e=>{
 if(!e.target.closest('[data-guest-start]'))return;
 openModal('<h2>로그인 없이 둘러볼까요?</h2><p>사주와 상담을 바로 이용할 수 있어요. 기록은 현재 브라우저에 연결된 서버 보관함에 저장되지만, 계정에는 연결되지 않아요.</p><p>브라우저 데이터를 지우거나 다른 기기로 접속하면 기존 기록을 찾기 어려울 수 있어요. 나중에 같은 브라우저에서 로그인하면 기록을 계정에 연결할 수 있어요.</p><button class="primary" id="guest-continue">알겠어요, 시작할게요</button><button class="secondary" id="guest-cancel">돌아가서 로그인하기</button>');
 $('#guest-cancel').onclick=()=>$('#modal').close();
 $('#guest-continue').onclick=()=>{$('#modal').close();nav(p()?'result':'birth');};
});
...
root.querySelectorAll('[data-pick]').forEach(b=>b.onclick=()=>{const id=b.dataset.pick;
 ...
 const cf=root.querySelector('#chat-form');if(cf){cf.onsubmit=e=>{e.preventDefault();plusOpen=false;sendMessage(cf.elements.message.value);};cf.elements.message.onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing){e.preventDefault();cf.requestSubmit();}};
 // 한 줄로 시작해 쓰는 만큼만 자랍니다. CSS의 max-height가 천장을 잡아요.
 const ta=cf.elements.message,grow=()=>{ta.style.height='auto';ta.style.height=ta.scrollHeight+'px';};
...
async function coachAvailable(){
 ...
 try{const {response,body}=await coachStatus();aiCoach=response.ok&&body.available===true;}catch{aiCoach=false;}
 return aiCoach;
}
...
const chartFor=c=>({name:p().name,gender:p().gender||'',pillars:result.pillars.map(v=>v.gz),dayStem:SK[result.day],
 ...
 topics:p().topics,initialConcern:p().question||'',today:E.dayName(today()).name+' '+E.dayName(today()).label+'일',
 flowAsOf:today(),periodFlow:E.flow(result,today()).slice(0,2).map(f=>({period:f.key,pillar:f.p.gz,god:f.god,stage:f.stage})),
...
async function epicAvailable(){
 ...
 try{const {response,body}=await epicStatus();aiEpic=response.ok&&body.available===true;}catch{aiEpic=false;}
 return aiEpic;
}
...
async function loadEpic(){
 ...
 try{
  const {response:r,body:j}=await requestEpic({name:p().name,pillars:result.pillars.map(v=>v.gz),dayStem:SK[result.day],
    element:ELEMENTS[result.element],strong:result.strong.map(i=>ELEMENTS[i]),weak:result.weak.map(i=>ELEMENTS[i]),
...
function capturedIds(){
 ...
 const ids=(currentConversation()?.messages||[]).map(m=>m.id);
 const i=ids.indexOf(capturing.a),j=capturing.b?ids.indexOf(capturing.b):i;
 if(i<0||j<0)return [];
 return ids.slice(Math.min(i,j),Math.max(i,j)+1);
}
function captureGuide(){
 if(!capturing?.a)return '캡처를 시작할 말풍선을 선택해 주세요.';
 if(!capturing.b)return '캡처를 종료할 말풍선을 선택해 주세요.';
 return capturedIds().length+'개 선택됨';
}
// 이미지를 클립보드에 넣는 것과 공유 시트는 기기마다 없을 수 있습니다.
...
// 고른 말풍선을 대화 순서대로 모아 이미지로 만듭니다.
async function saveCapture(how){
 const ids=capturedIds(),c=currentConversation();
 const picked=(c?.messages||[]).filter(m=>ids.includes(m.id));
 if(!picked.length)return;
 try{
  const blob=await captureMessages(picked,{title:'달빛 도령',subtitle:dateLabel(picked[0].at.slice(0,10))+' 상담'});
  ...
  if(how==='copy'){
   ...
   a.href=u;a.download='달빛사주-상담.png';a.click();
   setTimeout(()=>URL.revokeObjectURL(u),1000);notice('이미지로 저장했어요.');
  }
  capturing=null;render();
 }catch(e){
  if(e.name==='AbortError')return;
  notice(how==='copy'?'이미지를 복사하지 못했어요. 저장으로 받아 주세요.':'이미지를 만들지 못했어요. 다시 시도해 주세요.');
 }
}
...
async function requestAi(c,messages){let response,body;try{({response,body}=await requestCoachWithRetry({chart:chartFor(c),messages}));}catch(e){return {ok:false,reason:e.name==='AbortError'?'AI가 제한 시간 안에 답하지 못했습니다.':'연결 문제로 AI 답변을 받지 못했습니다.'};}if(response?.ok&&body?.text)return {ok:true,text:body.text,source:body.source,offer:body.offer};if(response?.status===429)return {ok:false,reason:'AI 사용량이 많습니다.'};if(body?.error?.includes('끊겼'))return {ok:false,reason:'AI 답변이 생성 중에 끊겼습니다.'};if(body?.error?.includes('형식')||body?.error?.includes('품질'))return {ok:false,reason:'AI 답변이 품질 검사를 통과하지 못했습니다.'};return {ok:false,reason:'AI 응답을 받지 못했습니다.'};}
...
async function sendMessage(text){text=text.trim();if(!text)return;if(text.length>1500){notice('메시지는 1,500자 이내로 입력해 주세요.');return;}const c=currentConversation();if(c.pending){notice('앞선 답변이 끝난 뒤 보내 주세요.');return;}if(c.profileKey!==profileKey()){notice('이전 사주의 상담입니다. 새 상담을 시작해 주세요.');return;}if(!await engineReady())return;c.messages.push({id:uid(),role:'user',text,at:new Date().toISOString()});
 ...
 if(!['safety','clarify'].includes(fallback.phase)&&await coachAvailable()){
  ...
  else{draft.text=fallback.text;draft.source='rules';draft.aiFailure=out.reason;}
...
async function action(a){if(a==='epic'){if(await engineReady())await loadEpic();return;}if(a==='retry'){if(!loaded)await load();else await persist();return;}if(a==='logout'){try{const {response}=await logout();if(!response.ok)throw new Error();clearTossLogin();authUser=null;page='welcome';await load();notice('로그아웃했어요.');}catch{notice('로그아웃하지 못했어요. 다시 시도해 주세요.');}return;}if(a==='export-json'){download('달빛-보관함.json',JSON.stringify(data,null,2),'application/json');return;}if(a==='export-text'){download('달빛-보관함.txt',exportText());return;}if(a==='chat-history'){chatHistoryModal();return;}
 if(a==='delete-all'){openModal(`<h2>${authUser?'계정과 개인정보를':'개인정보를'} 모두 삭제할까요?</h2><p>현재 보관함의 프로필·상담·선택 기록·저장한 답변이 삭제됩니다. 내려받은 파일과 캘린더 일정은 별도로 삭제해 주세요.</p><label class="check"><input id="delete-confirm" type="checkbox">복구할 수 없음을 이해했습니다.</label><p id="delete-error" role="alert"></p><button class="danger" id="delete-final">전체 삭제</button>`);$('#delete-final').onclick=async()=>{if(!$('#delete-confirm').checked){$('#delete-error').textContent='위 확인란을 선택해 주세요.';return;}$('#delete-final').disabled=true;await saveQueue;try{const {response}=authUser?await deleteAccount():await deleteJournal();if(!response.ok)throw new Error('삭제하지 못했어요. 잠시 후 다시 시도해 주세요.');generation++;authUser=null;data={profile:null,records:[],conversations:[],savedAnswers:[]};revision=0;result=null;activeConversation=null;localStorage.removeItem('dalbit-v2');saveState='ready';$('#modal').close();nav('welcome');notice('계정과 개인 보관함의 정보를 삭제했어요.');}catch(e){$('#delete-error').textContent=e.message;$('#delete-final').disabled=false;}};}}
async function load(){saveState='loading';refreshSave();try{const {response:res,body:r}=await getJournal();if(!res.ok)throw new Error(r.error);loaded=true;revision=r.revision;data=r.data||{profile:null,records:[],conversations:[],savedAnswers:[]};data.savedAnswers||=[];savedBaseline=JSON.parse(JSON.stringify(data));result=null;if(data.profile&&await engineReady()){try{result=E.calculate(data.profile);}catch{data.profile=null;notice('이전 프로필의 계산 정보를 다시 확인해 주세요.');}}if(afterLogin){page=result?'today':'birth';afterLogin=false;const url=new URL(location.href);url.searchParams.delete('auth');history.replaceState(null,'',url.pathname+url.search+url.hash);}saveError='';saveState=revision?'saved':'ready';render();const legacy=localStorage.getItem('dalbit-v2');if(legacy&&!r.data)offerMigration(legacy);}catch(e){markSaveError(e.message||'저장소에 연결하지 못했어요. 설정에서 다시 시도해 주세요.');render();}}
...
async function loadAuth(){
 await Promise.allSettled([
  authProviders().then(({response,body})=>{if(!response.ok)throw new Error('providers');authOptions=body;authStatus='ready';}).catch(()=>{authStatus='error';}),
  currentUser().then(({response,body})=>{if(response.ok)authUser=body.user||null;}).catch(()=>{})
 ]);
 if(loaded)render();
}
render();
...
document.addEventListener('click',async event=>{
 ...
 if(!link||!['/privacy/','/terms/','/support/','/account-deletion/'].includes(link.getAttribute('href')))return;
 event.preventDefault();
 const path=link.getAttribute('href');
 openModal('<p role="status">문서를 불러오고 있어요.</p>');
 $('#modal').dataset.document=path;
 try{
  const response=await fetch(path);
  if(!response.ok)throw new Error('document');
  const parsed=new DOMParser().parseFromString(await response.text(),'text/html');
  const main=parsed.querySelector('main');
  if(!main)throw new Error('document');
  if($('#modal').open&&$('#modal').dataset.document===path)openModal('<article class="legal-document">'+main.innerHTML+'</article>');
 }catch{
  if($('#modal').open&&$('#modal').dataset.document===path)openModal('<p>문서를 불러오지 못했어요. 잠시 후 다시 열어주세요.</p>');
 }
});
